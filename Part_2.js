class Rover {
    // Class Rover
    //Constructor (x-cordinate,y-cordinate,direction,obstacles)
    // Obstaces is Array of Arrays

    constructor(x, y, direction, obstacles = [[1, 4], [3, 5], [7, 4]]) {
        this.current_x = x;
        this.current_y = y;
    ...
    executeCommands(commandString) {
        ...
        console.log(`(${this.current_x}, ${this.current_y}) ${this.current_direction}`);
    }
    ...
    moveForward() {
        ...
        const next_x = this.current_x + (this.current_direction === "EAST" ? 1 : (this.current_direction === "WEST" ? -1 : 0));
        // If Moving Forward In Nourth direction y++  where y++ is the next point in y
        // If Moving Forward In South direction y--   where y-- is the next point in y
        const next_y = this.current_y + (this.current_direction === "NORTH" ? 1 : (this.current_direction === "SOUTH" ? -1 : 0));
        if (this.isObstacle(next_x, next_y)) {
            ...
            console.log(`(${this.current_x}, ${this.current_y}) ${this.current_direction} STOPPED due to collision`);
        } else {
    ...
    moveBackward() {
        ...
        const next_x = this.current_x - (this.current_direction === "EAST" ? 1 : (this.current_direction === "WEST" ? -1 : 0));
        // If Moving Backward In Nourth direction y--  where y-- is the next point in y
...
// BLFFF  Means Backward+Left+Forward+Forward+Forward

// OUTPUT (3, 4) NORTH STOPPED due to collision
// OUTPUT (3, 4) NORTH